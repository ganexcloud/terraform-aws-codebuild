...
variable "codebuild_source_s3_bucket_name" {
  description = "The location where the AWS CodeBuild project stores source resources. For type S3 the value must be a valid S3 bucket name/prefix. (Required when codebuild_source_type `type` is `CODEPIPELINE`)"
  type        = string
  default     = null
}
